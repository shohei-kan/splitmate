...
- Home から対象月の精算内容を固定 LINE グループへ手動通知可能
...
- LINE integration:
  - LINE webhook 受け口を追加し、`groupId` を安全寄りに取得できるようにした
  - Home から対象月の精算内容を LINE グループへ手動通知できるようにした
  - 月単位の通知状態保存と、通知済み UI を追加した
- Home UX:
  - `LINEで通知` を月ヘッダー右端の補助アクションとして整理した
  - 通知 UI をミニマル化し、詳細状態は hover / focus tooltip で確認できるようにした
  - 「今月の内訳」を 1 行アコーディオンに圧縮し、`集計を見る` は開いた時だけ見せる導線にした
